...
func updateReadme(cwd, oldModule, newModule, newApp string) {
	...
	b.WriteString("- Application logging through github.com/dan-sherwin/go-applog\n")
	b.WriteString("- Runtime logging/settings/DevLogBus integration through github.com/dan-sherwin/go-app-runtime\n")
	...
	b.WriteString("- Use `applog.Info`, `applog.Warn`, `applog.Error`, `applog.Debug`, and `applog.Debug2` through `applog.Debug5` in service code.\n")
	b.WriteString("- `go-applog` owns platform logging setup; `go-app-runtime` owns settings, RPC commands, and DevLogBus publishing.\n")
	b.WriteString("- Runtime settings: `log_level`, `devlogbus_enabled`, and `devlogbus_endpoint`.\n")
	b.WriteString("- Operator commands: `logging status`, `logging level`, and the `devlogbus` commands in the logging command group.\n")
	b.WriteString("- Standard log keys include app, version, commit, buildDate, pid, user, and error.\n\n")
